...
export default function Sessionsssss() {
    ...
    const unseenCount = React.useMemo(() => {
        return sessionLists.reduce((acc, s) => {
            const id = s.docId;
            if (id && !seenIds.has(id)) return acc + 1;
            return acc;
        }, 0);
    }, [sessionLists, seenIds]);
   ...
   React.useEffect(() => {
  // Expo Router's types don't include tab options on this object, so cast to any.
  (navigation as any).setOptions({
    tabBarBadge: unseenCount > 0 ? unseenCount : undefined, // remove when 0
