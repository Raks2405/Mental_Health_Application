...
export default function Profile() {
    ...
    const findIfUserIsAdmin = () => {
        if (adminEmails.includes(user?.email ?? '')) {
            return adminEmails[isAdminIndexNumber() ?? 0];
        }
        else {
            return user?.email;
        }
    }
    const emailId = findIfUserIsAdmin();
    ...
    return (
        <View style={styles.card}>
            <View style={styles.name_iconContainer}>
                ...
                <Text style={styles.name}>{emailId}</Text>
            </View>
