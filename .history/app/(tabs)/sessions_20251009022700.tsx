...
export default function Sessionsssss() {
    ...
    const handleEditButton = (session: Session) => {
        ...
        const combinedDateTimeString = `${session.date} ${session.time}`; 
    
    // 3. Create a single Date object from the combined string
    const combinedDateObject = new Date(combinedDateTimeString);

    // 4. Update the Date and Time state variables
    // We pass the SAME combined Date object to both setters.
    // The date picker will extract the date part, and the time picker will extract the time part.
    if (isNaN(combinedDateObject.getTime())) {
        // Fallback if parsing failed (e.g., if the stored format was inconsistent)
        Alert.alert("Error", "Could not parse stored date/time for editing.");
        setDate(new Date()); 
        setTime(new Date());
    } else {
        // Set the state using the successfully parsed Date object
        setDate(combinedDateObject); 
        setTime(combinedDateObject);
    }
