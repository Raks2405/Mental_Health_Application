...
import { Alert, KeyboardAvoidingView, Modal, Platform, Pressable, ScrollView, StyleSheet, Text, TextInput, View, Image, FlatList } from "react-native";
...
export default function Sessionsssss() {
    ...
    const [sessionLists, setSessionLists] = useState<Session[] | null>(null);
        ...
        return (
            <>
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        <View style={styles.subCard}>
                            <Pressable style={{ flexDirection: 'column' }} onPress={() => setSessionLists(item)}>
                ...
                <Modal
                    ...
                    transparent
                    animationType="slide"
                    onRequestClose={() => setSessionLists(null)}
                >
                    <Pressable style={styles.backdrop} onPress={() => setSessionLists(null)} />
                    <View style={styles.modalCard}>
                        <Text style={{ fontWeight: '700', fontSize: 18 }}>{sessionLists?.title}</Text>
                        <Text>{sessionLists?.date}</Text>
                        <Text>{sessionLists?.time}</Text>
                        <Text>{sessionLists?.location}</Text>
                        <Text style={{ marginTop: 8 }}>{sessionLists?.description}</Text>
                    </View>
                </Modal>
            </>
        );
