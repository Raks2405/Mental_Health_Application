...
// Remove conflicting Web SDK imports: EmailAuthProvider, reauthenticateWithCredential, updatePassword 
import React, { useState, useCallback } from "react";
import { Alert, KeyboardAvoidingView, Modal, Platform, Pressable, StyleSheet, Text, TextInput, View } from "react-native";
import { logOut } from '../../src/auth';
import { auth } from '../../src/firebase'; // This is the RNFB Auth instance
// Import RNFB EmailAuthProvider for credentials
...
    useFocusEffect(
    useCallback(() => {
      const u = authNative().currentUser; // Use RNFB method to get current user
...
        // Get the currently logged-in RNFB User instance
        const userInstance = authNative().currentUser;
        
        // Check if the user exists and has an email (not anonymous)
        if (!userInstance || !userInstance.email) {
            return Alert.alert('Error', 'No authenticated user with an email address found.');
        }

        try {
            setSaving(true);
            
            // 1. Create the credential using RNFB EmailAuthProvider (needs RNFB syntax)
            const cred = authNative.EmailAuthProvider.credential(userInstance.email, currPwd);
            
            // 2. RNFB: Call reauthenticateWithCredential and updatePassword on the USER INSTANCE
            await userInstance.reauthenticateWithCredential(cred); // required by Firebase
            await userInstance.updatePassword(newPwd);
            
            setShowPwdScreen(false);
            setCurrPwd(''); setNewPwd(''); setConfirmPwd('');
            Alert.alert('Success', 'Password changed');
        } catch (e) {
            // In case of wrong password, Firebase throws an error (auth/wrong-password)
...
                <Text style={styles.name}>{email} {isAdmin ? "(Admin)" : ""}</Text>
            </View>
