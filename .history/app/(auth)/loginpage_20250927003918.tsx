...
export default function LoginPage() {
  ...
  const adminEmail = ["admin@uco.edu", "admin2@uco.edu"];
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      adminEmail.includes(email.trim()) ? storeUsername('Admin') : storeUsername(email);                                 
    } catch (err: any) {
  ...
  const storeUsername = (name?: string) => {
    // navigate to the tabs layout root and pass a global param 'username'
    // so all screens under the (tabs) layout can read it with useGlobalSearchParams
    router.replace({
      pathname: '/(tabs)/home',
      params: {
        username: name,
