...
import type { VideoItem } from "../data/resources";
import { useRecommendation } from "../context/RecommendationContext";
import { CATEGORY_QUERIES } from "../data/resourceQueries";
import { fetchVideosByCategory, getCachedVideos } from "../data/resourceFetch";
import {
...
const buildRankingPrompt = (
  description: string,
  items: Array<{ id: number; title: string; channel: string }>
) => {
  return [
    "You are a recommender. Pick the top 10 videos most relevant to the user's description.",
    'Return ONLY a JSON array of video ids, like [1,2,3]. No extra text.',
    `User description: """${description.trim()}"""`,
    "Videos:",
    ...items.map((item) => `${item.id}. ${item.title} — ${item.channel}`),
  ].join("\n");
};
...
export default function VideosScreen() {
  ...
  const [remoteVideos, setRemoteVideos] = useState<VideoItem[]>([]);
  ...
  const videoCategories = useMemo(() => {
    return Object.fromEntries(
      Object.entries(resourceCategories ?? {}).map(([key, category]) => [
        key,
        { label: category.label, videos: category.videos },
      ])
    ) as Record<string, { label: string; videos: VideoItem[] }>;
  }, [resourceCategories]);

  const allVideos = useMemo(() => {
    const unique = new Map<string, VideoItem>();
    Object.values(videoCategories).forEach((category) => {
  ...
  useEffect(() => {
    ...
    const query = CATEGORY_QUERIES[categoryKey]?.videos ?? categoryKey;
    const cached = getCachedVideos(categoryKey, query);
    if (cached) {
    ...
    setIsLoading(true);
    setRemoteError(null);
    fetchVideosByCategory(categoryKey, query)
      .then((videos) => {
        if (!isActive) return;
      ...
      .catch((err) => {
        if (!isActive) return;
        setRemoteError(err instanceof Error ? err.message : "Failed to load videos.");
      })
      .finally(() => {
        if (!isActive) return;
        setIsLoading(false);
  ...
  const categoryLabel = showUnavailable
    ...
    : categoryKey
    ? videoCategories[categoryKey]?.label ?? "General resources"
    : "General resources";
  const videos = useMemo(() => {
    if (!categoryKey) return allVideos;
    const base = videoCategories[categoryKey]?.videos ?? [];
    const unique = new Map<string, VideoItem>();
    [...base, ...remoteVideos].forEach((video) => {
      if (!unique.has(video.videoId)) {
        unique.set(video.videoId, video);
      }
    });
    return Array.from(unique.values()).slice(0, 100);
  }, [allVideos, categoryKey, remoteVideos, videoCategories]);
  ...
  useEffect(() => {
    ...
    const rankingText =
      queryText?.trim() || videoCategories[categoryKey]?.label || categoryKey;
    const cached = videoRankings[categoryKey];
    ...
    const fallbackKeys = () => {
      ...
      videos.slice(0, 10).forEach((video) => keys.add(video.videoId));
      return keys;
    };

    const runRanking = async () => {
      const apiKey = process.env.EXPO_PUBLIC_GEMINI_API_KEY;
      const candidates = videos.slice(0, 40).map((video, index) => ({
        id: index + 1,
        title: video.title,
      ...
      if (candidates.length === 0) {
        ...
        setVideoRanking(categoryKey, rankingText, Array.from(keys));
        return;
      }
      ...
      try {
        ...
        candidates.forEach((candidate, idx) => {
          if (idSet.has(candidate.id)) {
            ...
            if (video) keys.add(video.videoId);
          }
        });
        if (isActive) {
          const finalKeys = keys.size > 0 ? keys : fallbackKeys();
          setRecommendedKeys(finalKeys);
          setVideoRanking(categoryKey, rankingText, Array.from(finalKeys));
        }
      } catch (err) {
        if (isActive) {
          const keys = fallbackKeys();
  ...
  }, [categoryKey, queryText, setVideoRanking, showUnavailable, videoCategories, videoRankings, videos]);

  const orderedVideos = useMemo(() => {
    if (recommendedKeys.size === 0) return videos;
    const recommended: typeof videos = [];
    const rest: typeof videos = [];
    videos.forEach((video) => {
      if (recommendedKeys.has(video.videoId)) {
  ...
  }, [recommendedKeys, videos]);

  const totalPages = Math.max(1, Math.ceil(orderedVideos.length / PAGE_SIZE));
  const clampedPage = Math.min(page, totalPages);
  const pagedVideos = useMemo(() => {
    const start = (clampedPage - 1) * PAGE_SIZE;
    return orderedVideos.slice(start, start + PAGE_SIZE);
  }, [clampedPage, orderedVideos]);
  const pagination = !showUnavailable && orderedVideos.length > 0 ? (
    ...
    <View style={s.container}>
      ...
      <ScrollView contentContainerStyle={s.body}>
        ...
        {!showUnavailable && categoryKey && isLoading ? (
          <Text style={s.loading}>Loading more videos...</Text>
        ) : null}
        ...
        {showUnavailable ? (
        ...
        ) : !isLoading && pagedVideos.length === 0 ? (
          <View style={s.emptyCard}>
            <Text style={s.emptyTitle}>No videos found</Text>
            <Text style={s.emptyText}>We don't have videos for this category yet.</Text>
          </View>
        ) : (
          pagedVideos.map((video) => (
            <VideoCard
              ...
              recommended={recommendedKeys.has(video.videoId)}
            />
          ))
        )}
        {pagination}
